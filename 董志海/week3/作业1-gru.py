...
class CharGRUDataset(Dataset):
    ...
    该类用于将文本数据转换为适合GRU模型训练的格式，包括字符编码、填充等预处理操作。
    """
...
def classify_text_gru(text, model, char_to_index, max_len, index_to_label):
    """
    使用训练好的GRU模型对文本进行分类预测

    Args:
        text (str): 待分类的文本
        model (GRUClassifier): 训练好的GRU模型
        char_to_index (dict): 字符到索引的映射字典
        max_len (int): 文本最大长度
