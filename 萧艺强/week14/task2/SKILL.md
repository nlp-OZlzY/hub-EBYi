...
'''
作业2: 定义一个skill，包含对股票的可视化功能，对于股票的周波动、日波动绘制在一个图中，
    并基于波动大小给出一个买入卖出的最佳时间建议；
    复用已有的skill， 写一个类似功能的skill
'''
# 股票可视化方法

##  一、股票可视化的数据和图
- 查询股票的周K线（最近的10周），日k线(最近的30日)数据
- 使用python脚本在同一个图中绘制周K线，日k线图
- 绘制好的图按 股票代码.png 保存在assets的文件夹
- 使用**py312**环境执行

## 二、买入卖出建议
   参考**周K线，日k线**数据分析以下维度数据
- 数据的波动大小，比例


综合上述指标，给出需要做买入或者卖出的建议和时间点
