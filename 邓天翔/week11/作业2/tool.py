...
# 查询用户信息
@mcp.tool
def query_user_info(user_name: Annotated[str, "用户名"]):
...
# 查询用户婚姻状态
@mcp.tool
def query_user_marriage(user_name: Annotated[str, "用户名"]):
    """Query user marriage status based on the username."""
    if len(user_name) == 2:
        return "未婚"
    else:
        return "已婚"
...
# 查询用户的等级
@mcp.tool
def query_user_level(user_name: Annotated[str, "用户名"]):
    """Query user level based on the username."""
    if len(user_name) == 2:
        return "普通用户"
    elif len(user_name) == 3:
        return "中级用户"
