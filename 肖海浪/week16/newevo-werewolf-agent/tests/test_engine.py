...
class TestGameSummaryPhase:
    ...
    async def test_run_summaries_called_once(self):
        ...
        if engine._summary_task is not None:
            await engine._summary_task

        summary_done = await engine.step()
        assert summary_done["step_data"].get("summaries_complete") is True
