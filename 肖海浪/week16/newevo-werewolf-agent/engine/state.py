...
维护狼人杀游戏的完整状态，包括玩家列表、当前阶段、天数、
投票记录、夜晚死亡、对话历史等。提供信息隔离的上下文查询。
"""
...
class GameState:
    ...
    def get_player_private_context(self, player_id: int) -> Dict[str, Any]:
        """获取某个玩家的私有上下文（信息隔离的核心实现）

        将公开信息（所有人可见）和私有信息（仅本人可见）合并，
        并通过 _filter_visible_dialogues 过滤对话历史，实现信息隔离。
        """
    ...
    def _filter_visible_dialogues(self, player: Player) -> List[Dict[str, Any]]:
        ...
        for d in self.dialogues:
            ...
            # 私有对话：只有本人能看到
                ...
                visible.append(d)
            # 狼人可以看到队友的决策
            elif d["action"] == "wolf_vote" and player.role_type == RoleType.WEREWOLF:
                # 检查是否同为狼人
