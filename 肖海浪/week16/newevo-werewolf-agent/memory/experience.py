...
实现跨游戏的经验积累与复用，是自演化系统的重要补充：
1. 每局游戏结束后，SummaryAgent 为每个玩家生成结构化总结
2. 总结通过 save_experience 持久化到 memory/experiences/{role_type}.json
3. 下一局游戏开始时，PlayerAgent 通过 get_experience_prompt 注入最近 3 条经验

与 PromptStore 的区别：
- PromptStore 存储角色的「策略指令」（由 SelfReflector 优化）
- Experience 存储角色的「过往教训」（由 SummaryAgent 生成）
- 两者同时注入 PlayerAgent 的 prompt，形成策略 + 经验的双重指导
"""
