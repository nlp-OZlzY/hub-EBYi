...
LOCAL_MODEL_PATH = r"D:\BaiduNetdiskDownload\models\Qwen\Qwen3-0.6B"  # ← 修改这里为你的本地模型路径
...
if CHOOSE_MODEL == "0.6B":
    ...
    for filename in set(index["weight_map"].values()):
        ...
        if not os.path.exists(shard_path):
            raise FileNotFoundError(f"权重分片文件不存在: {shard_path}")
        shard = load_file(shard_path)
        weights_dict.update(shard)
...
class Qwen3Tokenizer:
    ...
    def __init__(self, tokenizer_file_path=r"D:\BaiduNetdiskDownload\models\Qwen\Qwen3-0.6B\tokenizer.json", repo_id=None,
...
if not os.path.exists(tokenizer_file_path):
    raise FileNotFoundError(f"Tokenizer 文件不存在: {tokenizer_file_path}")

if USE_REASONING_MODEL or USE_INSTRUCT_MODEL:
    tokenizer = Qwen3Tokenizer(
        tokenizer_file_path=tokenizer_file_path,
        repo_id=None,  # 本地模式不需要 repo_id
        apply_chat_template=True,
        add_generation_prompt=True,
        add_thinking=USE_REASONING_MODEL
    )
