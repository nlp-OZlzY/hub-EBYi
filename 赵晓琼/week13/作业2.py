...
if USE_REASONING_MODEL or USE_INSTRUCT_MODEL:
    repo_id = f"./Qwen3-0.6B/Qwen/Qwen3-{CHOOSE_MODEL}"
else:
    repo_id = f"./Qwen3-0.6B/Qwen/Qwen3-{CHOOSE_MODEL}-Base"
...
if CHOOSE_MODEL == "0.6B":
    ...
    #     local_dir=local_dir,
    # )
    repo_dir = repo_id  # 期望 repo_dir/model.safetensors 存在
    weights_path = os.path.join(repo_dir, "model.safetensors")
    if not os.path.exists(weights_path):
        raise FileNotFoundError(f"Expected local weights at: {weights_path}")
...
if USE_REASONING_MODEL:
    tokenizer_file_path = f"./Qwen3-0.6B/Qwen/Qwen3-{CHOOSE_MODEL}/tokenizer.json"
else:
    tokenizer_file_path = f"./Qwen3-0.6B/Qwen/Qwen3-{CHOOSE_MODEL}-Base/tokenizer.json"
...
#     local_dir=local_dir,
# )
tokenizer_file_path = os.path.join(repo_dir, "tokenizer.json")
if not os.path.exists(tokenizer_file_path):
    raise FileNotFoundError(f"Expected local tokenizer at: {tokenizer_file_path}")
